...
- **Advanced ML Strategy**:
    ...
    - **特徴量** (30個):
        ...
        - **Context Features**: コース適性、天候、距離、馬場状態、出走頭数、馬齢を考慮。
        - **Lag Features (過去走)**: 直近3走の着順・平均着順、前走のスピード指数・**上がり3F**・出走間隔・脚質を推論時に動的に参照。
        - **Aptitude (適性)**: コース種別（芝/ダート）・距離カテゴリ別の勝率。
    - **ベッティング戦略**: 「勝利確率 × オッズ」（期待値ベース）によるスコアリング。
        - 期待回収率を最大化するように算出されたスコアで馬を選別。

- **Backtesting & Verification**:
    - `evaluate.yml` の入力パラメータによる詳細な条件設定（レース番号範囲等）。
    - **Range Evaluation**: 各種パラメータを範囲指定して一括検証。
